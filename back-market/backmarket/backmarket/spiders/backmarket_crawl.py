...
class BackmarketCrawlSpider(scrapy.Spider):
    ...
    def parse_product(self, response):
        ...
        try:
            item['fair'] = response.css('li:nth-child(1) > a > div > div.body-2-light.text-center.text-primary-light::text').get(default = "No data").strip().replace('\n', '').replace('£', '')
        except:
            item['fair'] = "No data"
        try:
            item['good']= response.css('li:nth-child(2) > a > div > div.body-2-light.text-center.text-primary-light::text').get(default = "No data").strip().replace('\n', '').replace('£', '')
        except:
            item['good'] = "No data"
        try:
            item['excellent'] = response.css('li:nth-child(3) > a > div > div.body-2-light.text-center.text-primary-light::text').get(default = "No data").strip().replace('\n', '').replace('£', '')
        except:
            item['excellent'] = "No data"
        try:
            item['rating'] =response.css('div.hidden.md\:flex.flex-grow.flex-col > button > div > span::text').get(default = "No data").strip().replace('\n', '')
        except:
            item['rating'] = "No data"
        try:
            item['reviews'] = response.css('div.hidden.md\:flex.flex-grow.flex-col > button > span::text').get(default = "No data").strip().replace('\n', '')
        except:
